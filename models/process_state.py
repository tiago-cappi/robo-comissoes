...
class ProcessStateManager:
    ...
    def __init__(
        self,
        estado_df: Optional[pd.DataFrame] = None,
        filepath: str = "Estado_Processos_Recebimento.xlsx",
    ):
        """
    ...
    def _normalize_estado(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "STATUS_PAGAMENTO" in df.columns:
            df["STATUS_PAGAMENTO"] = (
                df["STATUS_PAGAMENTO"].astype(str).replace({"nan": None})
            )
        if "STATUS_RECONCILIACAO" in df.columns:
            df["STATUS_RECONCILIACAO"] = (
                df["STATUS_RECONCILIACAO"].astype(str).replace({"nan": None})
            )
        if "STATUS_CALCULO_MEDIAS" in df.columns:
            df["STATUS_CALCULO_MEDIAS"] = (
                df["STATUS_CALCULO_MEDIAS"].astype(str).replace({"nan": None})
            )
        if "MES_ANO_FATURAMENTO" in df.columns:
            df["MES_ANO_FATURAMENTO"] = (
                df["MES_ANO_FATURAMENTO"].astype(str).replace({"nan": None})
            )
    ...
    def update_payment_received(
        ...
        if len(indices) == 0:
            ...
            nova_linha = {
                ...
                "VALOR_TOTAL_PROCESSO": (
                    valor_total_processo if valor_total_processo is not None else 0.0
                ),
            ...
            self.estado = pd.concat(
                [self.estado, pd.DataFrame([nova_linha])], ignore_index=True, sort=False
            )
        else:
            ...
            pago_anterior = pd.to_numeric(
                self.estado.at[idx, "TOTAL_PAGO_ACUMULADO"], errors="coerce"
            )
            pago_anterior = float(pago_anterior) if not pd.isna(pago_anterior) else 0.0
            self.estado.at[idx, "TOTAL_PAGO_ACUMULADO"] = pago_anterior + float(
                valor_recebido
            )
            ...
            if valor_total_processo is not None:
                valor_atual = pd.to_numeric(
                    self.estado.at[idx, "VALOR_TOTAL_PROCESSO"], errors="coerce"
                )
                if pd.isna(valor_atual) or valor_atual == 0.0:
                    self.estado.at[idx, "VALOR_TOTAL_PROCESSO"] = float(
                        valor_total_processo
                    )
    ...
    def update_payment_advanced(
        ...
        if len(indices) == 0:
            ...
            antecipacoes_anterior = pd.to_numeric(
                self.estado.at[idx, "TOTAL_ANTECIPACOES"], errors="coerce"
            )
            antecipacoes_anterior = (
                float(antecipacoes_anterior)
                if not pd.isna(antecipacoes_anterior)
                else 0.0
            )
            self.estado.at[idx, "TOTAL_ANTECIPACOES"] = antecipacoes_anterior + float(
                valor_recebido
            )

            # Recalcular total pago acumulado
            pagtos_regulares = pd.to_numeric(
                self.estado.at[idx, "TOTAL_PAGAMENTOS_REGULARES"], errors="coerce"
            )
            pagtos_regulares = (
                float(pagtos_regulares) if not pd.isna(pagtos_regulares) else 0.0
            )
            self.estado.at[idx, "TOTAL_PAGO_ACUMULADO"] = (
                antecipacoes_anterior + float(valor_recebido) + pagtos_regulares
            )
    ...
    def update_payment_regular(
        ...
        if len(indices) == 0:
            ...
            pagtos_anterior = pd.to_numeric(
                self.estado.at[idx, "TOTAL_PAGAMENTOS_REGULARES"], errors="coerce"
            )
            pagtos_anterior = (
                float(pagtos_anterior) if not pd.isna(pagtos_anterior) else 0.0
            )
            self.estado.at[idx, "TOTAL_PAGAMENTOS_REGULARES"] = pagtos_anterior + float(
                valor_recebido
            )

            # Recalcular total pago acumulado
            antecipacoes = pd.to_numeric(
                self.estado.at[idx, "TOTAL_ANTECIPACOES"], errors="coerce"
            )
            antecipacoes = float(antecipacoes) if not pd.isna(antecipacoes) else 0.0
            self.estado.at[idx, "TOTAL_PAGO_ACUMULADO"] = (
                antecipacoes + pagtos_anterior + float(valor_recebido)
            )
    ...
    def update_commission_advanced(self, processo_id, valor_comissao: float) -> None:
        ...
        if len(indices) == 0:
            ...
            idx = indices[0]
            adiantado_anterior = pd.to_numeric(
                self.estado.at[idx, "TOTAL_ADIANTADO_COMISSAO"], errors="coerce"
            )
            adiantado_anterior = (
                float(adiantado_anterior) if not pd.isna(adiantado_anterior) else 0.0
            )
            self.estado.at[idx, "TOTAL_ADIANTADO_COMISSAO"] = (
                adiantado_anterior + float(valor_comissao)
            )
            self.estado.at[idx, "ULTIMA_ATUALIZACAO"] = datetime.now().isoformat()
    ...
    def update_process_status(
        ...
        if len(indices) > 0:
            ...
            if status_processo_analise is not None:
                self.estado.at[idx, "STATUS_PROCESSO_ANALISE"] = str(
                    status_processo_analise
                )
    ...
    def update_process_metrics(
        ...
        status_calculo_medias: Optional[str] = "REALIZADO",
    ) -> None:
        """
        ...
        if len(indices) == 0:
            # Não criar novos processos aqui; estado deve refletir apenas processos vindos de Análise Financeira
            # Este método apenas atualiza métricas de processos já existentes no estado.
            return
        ...
        if tcmp_por_colaborador is not None:
            self.estado.at[idx, "TCMP"] = json.dumps(
                tcmp_por_colaborador, ensure_ascii=False
            )
        if fcmp_por_colaborador is not None:
            self.estado.at[idx, "FCMP"] = json.dumps(
                fcmp_por_colaborador, ensure_ascii=False
            )
    ...
    def get_eligible_for_reconciliation(self) -> pd.DataFrame:
        ...
        estado_norm["_status_pag_norm"] = estado_norm["STATUS_PAGAMENTO"].apply(
            normalize_text
        )
        estado_norm["_status_analise_norm"] = estado_norm[
            "STATUS_PROCESSO_ANALISE"
        ].apply(normalize_text)
        estado_norm["_status_reconc_norm"] = estado_norm["STATUS_RECONCILIACAO"].apply(
            normalize_text
        )
        ...
        mask_nao_reconciliado = ~estado_norm["_status_reconc_norm"].isin(
            ["REALIZADA", "CONCLUIDA"]
        )

        eligible = self.estado[
            mask_quitado & mask_faturado & mask_nao_reconciliado
        ].copy()
    ...
    def get_process_summary(self) -> Dict:
        ...
        return {
            ...
            "processos_quitados": int(
                (
                    self.estado["STATUS_PAGAMENTO"]
                    .apply(normalize_text)
                    .str.contains("QUITADO", na=False)
                ).sum()
            ),
            "processos_reconciliados": int(
                (
                    self.estado["STATUS_RECONCILIACAO"].apply(normalize_text)
                    == "REALIZADA"
                ).sum()
            ),
            "processos_elegiveis_reconciliacao": len(
                self.get_eligible_for_reconciliation()
            ),
        }
