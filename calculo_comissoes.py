...
class CalculoComissao:
    ...
    def _carregar_dados(self):
        ...
        try:
            ...
            # Carregar pagamentos regulares do mês (opcional - NOVO)
            try:
                self.data["PAGAMENTOS_REGULARES"] = pd.read_excel(
                    ARQUIVO_PAGAMENTOS_REGULARES
                )
            except Exception:
                self.data["PAGAMENTOS_REGULARES"] = pd.DataFrame(
                    columns=[
    ...
    def _reconciliar_e_calcular_metricas_do_mes(self):
        ...
        """
        try:
            _info("[Métricas/Reconciliação] Início do cálculo de métricas e reconciliações do mês")
            df_anal = self.data.get("ANALISE_COMERCIAL_COMPLETA", pd.DataFrame())
            if df_anal.empty:
                _info(
            ...
            if LOG_VERBOSE:
                _info(f"[Métricas/Reconciliação] Processos faturados existentes no ESTADO: {len(processos)}")
            if not processos:
            ...
            _info(f"[Métricas/Reconciliação] Processando {total_proc} processo(s) elegível(eis)")
            for idx_proc, proc in enumerate(processos, start=1):
                try:
                    if LOG_VERBOSE and (idx_proc % 50 == 0 or idx_proc == 1 or idx_proc == total_proc):
                        _info(f"[Métricas/Reconciliação] Progresso {idx_proc}/{total_proc} (proc={proc})")
                    ...
                    try:
                        ...
                        if len(indices) > 0:
                            ...
                            if LOG_VERBOSE:
                                _info(f"[Métricas/Reconciliação] (DEBUG) Processo {proc_str} fora do ESTADO; detalhes de métricas não salvos.")
                    except Exception as e:
                        if hasattr(self, "_logger"):
                    ...
                    if total_adiantado > 0:
                        ...
                        try:
                            ...
                            if len(indices) > 0:
                                ...
                                self.state_manager.estado.loc[idx, 'DETALHES_CALCULO_RECONCILIACAO'] = detalhes_reconciliacao_json
                            else:
                                if LOG_VERBOSE:
                                    _info(f"[Métricas/Reconciliação] (DEBUG) Processo {proc_str} fora do ESTADO; detalhes de reconciliação não salvos.")
                        except Exception as e:
                            if hasattr(self, "_logger"):
                ...
                try:
                    ...
                    if logs_eventos:
                        ...
                        if len(indices) > 0:
                            ...
                            if LOG_VERBOSE:
                                _info(f"[Métricas/Reconciliação] (DEBUG) Processo {proc_str} fora do ESTADO; LOG_EVENTOS não salvo.")
                except Exception as e:
                    if hasattr(self, "_logger"):
            ...
            _info(f"[Métricas/Reconciliação] Finalizado: {len(resumo_list)} processo(s) com resumo de reconciliação")
        except Exception as e:
            self._log_validacao(
    ...
    def _calcular_comissoes_recebimento_nova_logica(self):
        ...
        """
        try:
            _info("[Recebimentos] Início do cálculo de comissões por recebimento (nova lógica)")
            ...
            if pagamentos.empty:
                self.comissoes_recebimento_df = pd.DataFrame()
                _info("[Recebimentos] Nenhum pagamento encontrado em Análise Financeira.xlsx")
                return
            ...
            for _, row in pagamentos.iterrows():
                ...
                if LOG_VERBOSE and ((cont_adiant + cont_reg) % 200 == 0):
                    ...
                    _info(f"[Recebimentos] Progresso: {cont_adiant + cont_reg}/{total_rows} processados "
            ...
            for processo in processos_com_pagamentos:
                try:
                    ...
                    if len(indices) > 0:
                        ...
                        if LOG_VERBOSE:
                            _info(f"[Recebimentos] (DEBUG) Processo {processo_str} fora do ESTADO; debug de pagamentos não salvo.")
                except Exception as e:
                    # Não quebrar o cálculo se o log falhar
                    if hasattr(self, "_logger"):
            ...
            decorrido_total = (fim_ts - inicio_ts).total_seconds() if 'inicio_ts' in locals() else None
            _info(f"[Recebimentos] Finalizado: processados {total_proc_calc}/{total_rows} pagamentos "
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            with _timer_ctx("Executar scripts de limpeza", _safe_percent("limpezas")):
                if skip_clean:
                    _info(
                    ...
                    if r1.returncode != 0:
                        msg = f"ERRO: o script 'limpeza_recebimentos.py' retornou código {r1.returncode}. Abortando."
                        print(msg)
                        _tracker_abort(msg)
                    if not os.path.exists("Recebimentos_do_Mes.xlsx"):
                        msg = "ERRO: arquivo 'Recebimentos_do_Mes.xlsx' não foi gerado pelo script de limpeza. Abortando."
                        print(msg)
                        _tracker_abort(msg)
                    ...
                    if r2.returncode != 0:
                        msg = f"ERRO: o script 'limpeza_status_pagamentos.py' retornou código {r2.returncode}. Abortando."
                        print(msg)
                        _tracker_abort(msg)
                    if not os.path.exists("Status_Pagamentos_Processos.xlsx"):
                        msg = "ERRO: arquivo 'Status_Pagamentos_Processos.xlsx' não foi gerado pelo script de limpeza. Abortando."
                        print(msg)
                        _tracker_abort(msg)

                    _info(
                        f"Executando script de limpeza da análise financeira para {mes}/{ano}..."
                    )
                    r3 = subprocess.run(
                        [
                            sys.executable,
                            "limpeza_analise_financeira.py",
                            str(mes),
                            str(ano),
                        ],
                        text=True,
                    ...
                    if r3.returncode != 0:
                        msg = f"ERRO: o script 'limpeza_analise_financeira.py' retornou código {r3.returncode}. Abortando."
                        print(msg)
                        _tracker_abort(msg)
                    if not os.path.exists("Pagamentos_Regulares_do_Mes.xlsx"):
                        msg = "ERRO: arquivo 'Pagamentos_Regulares_do_Mes.xlsx' não foi gerado pelo script de limpeza. Abortando."
                        print(msg)
                        _tracker_abort(msg)

                    _info("Scripts de limpeza executados com sucesso.")
        except Exception as e:
            msg = f"AVISO: falha ao executar os scripts de limpeza automaticamente: {e}. Abortando."
            print(msg)
            _tracker_abort(msg)
