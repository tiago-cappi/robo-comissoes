...
def gerar_conversoes(df, mes, ano):
    ...
    filtro = (df[data_aceite_col].dt.month == mes) & (df[data_aceite_col].dt.year == ano)
    df_filtrado = df[filtro].copy()
    ...
    if op_col is not None:
        ...
        if 'operacao_codigo' in df_filtrado.columns:
            df_filtrado = df_filtrado.drop(columns=['operacao_codigo'])
        after_ops = len(df_filtrado)
        print(f"Filtragem por operação aplicada: {before_ops} -> {after_ops} linhas (apenas operações válidas mantidas).")
    else:
    ...
    if not available:
        print("AVISO: nenhuma das colunas desejadas foi encontrada para montar 'Conversões'.")
        return False
...
def run_preparador(mes: int, ano: int) -> bool:
    ...
    sep_detected, used_enc_for_sep = _detect_sep(ARQUIVO_ANALISE_COMPLETA, encodings_to_try)
    for enc in encodings_to_try:
        try:
            ...
            print(f"Arquivo lido com sucesso com encoding={enc} and sep='{sep_detected}'.")
            break
        except Exception as e:
            last_exc = e
...
def gerar_faturados_ytd(df, mes, ano):
    ...
    if status_col is None:
        print("AVISO: coluna 'Status Processo' não encontrada; nenhum filtro de status será aplicado.")
    else:
    ...
    if op_col is not None:
        def op_allowed(val):
            ...
            if n in allowed_full:
                return True
            # check prefix code
            code = s.strip().split(' - ')[0].split()[0].upper() if s.strip()!='' else ''
...
def gerar_retencao_clientes(df, mes, ano):
    ...
    if status_col:
        ...
        print('AVISO: Status Processo não encontrado; assumindo todas as linhas possíveis.')
...
def main():
    ...
    for c in date_candidates:
        nc = _norm(c)
        if ('dt' in nc and ('emiss' in nc or 'emissao' in nc)) or 'data' in nc and 'emiss' in nc:
            date_col = c
            break

    # se não encontrou, tentar algumas outras colunas candidatas comuns
    if date_col is None:
        for alt in ['Data Aceite', 'Dt Entrada', 'Dt Encerramento', 'Dt Emissao', 'Dt Emissao']:
    ...
    if date_col is None:
        print(f"ERRO: Não foi possível localizar coluna de data 'Dt Emissão' no arquivo '{ARQUIVO_ANALISE_COMPLETA}'. Colunas encontradas: {list(df_analise.columns)}")
        sys.exit(1)
