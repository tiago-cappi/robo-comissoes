...
class PaymentMapper:
    ...
    def __init__(self, analise_comercial_df: pd.DataFrame):
        ...
        self.processo_col = self.column_finder.find_column(
            ["processo", "id processo", "id_processo"]
        )
        self.valor_col = self.column_finder.find_column(
            ["valor realizado", "valor_realizado", "valorrealizado"]
        )
        self.cliente_col = self.column_finder.find_column(
            ["cliente", "id cliente", "id_cliente"]
        )

        # Cache de processos normalizados para performance
        if self.processo_col:
            self._processo_normalized = self.analise_df[self.processo_col].apply(
    ...
    def map_payment(
        self, processo_val, valor_val: float = None, id_cliente_val=None
    ...
    def map_payment_by_nf(
        self, documento_normalizado, valor_pago: float = None, id_cliente=None
    ) -> Tuple[Optional[pd.Series], str]:
        """
        Mapeia um pagamento regular usando NUMERO NF.

        Busca processos que tenham NUMERO NF correspondente aos 6 primeiros dígitos
        do documento.

        Args:
            documento_normalizado: 6 primeiros dígitos do documento
            valor_pago: Valor pago (opcional, para validação/desempate)
            id_cliente: ID do cliente (opcional, para validação adicional)

        Returns:
            Tupla (linha_mapeada, método_usado):
                - linha_mapeada: pd.Series com a linha encontrada, ou None
                - método_usado: string indicando o método
        """
        # Buscar coluna NUMERO NF
        nf_col = self.column_finder.find_column(
            ["numero nf", "numero_nf", "numeronf", "nf", "numero nota", "numero_nota"]
        )

        if nf_col is None:
            return None, "no_nf_column"

        # Normalizar documento (preservar zeros à esquerda)
        doc_norm = str(documento_normalizado).strip()

        if not doc_norm:
            return None, "invalid_documento"

        # Estratégia 1: Match exato (6 primeiros dígitos)
        # IMPORTANTE: Converter coluna NF para string e pegar primeiros 6 dígitos
        nf_normalized = self.analise_df[nf_col].astype(str).str.strip().str[:6]
        mask = nf_normalized == doc_norm
        matches = self.analise_df[mask]

        if len(matches) == 1:
            return matches.iloc[0], "exact_match_nf"
        elif len(matches) > 1:
            # Desempate por valor ou cliente
            best_match = self._choose_best_match_nf(matches, valor_pago, id_cliente)
            return best_match, "exact_match_nf_multi"

        # Estratégia 2: Match por substring (caso NF contenha o documento)
        substring_mask = nf_normalized.apply(
            lambda x: doc_norm in str(x) if pd.notna(x) and str(x) else False
        )
        candidates = self.analise_df[substring_mask]

        if not candidates.empty:
            if len(candidates) == 1:
                return candidates.iloc[0], "substring_match_nf"
            else:
                best_match = self._choose_best_match_nf(
                    candidates, valor_pago, id_cliente
                )
                return best_match, "substring_match_nf_multi"

        # Nenhuma estratégia funcionou
        return None, "not_found"

    def _choose_best_match_nf(
        self, candidates: pd.DataFrame, valor_pago: Optional[float], id_cliente
    ) -> pd.Series:
        """
        Escolhe o melhor match entre múltiplos candidatos usando valor e cliente.
        ...
        Returns:
            Melhor candidato (Series)
        """
        # Se tem ID do cliente, filtrar por ele primeiro
        if id_cliente is not None and self.cliente_col is not None:
            cliente_normalized = str(id_cliente).strip()
            cliente_mask = (
                candidates[self.cliente_col].astype(str).str.strip()
                == cliente_normalized
            )
            cliente_matches = candidates[cliente_mask]

            if not cliente_matches.empty:
                candidates = cliente_matches

        # Se ainda há múltiplos e temos valor, escolher o mais próximo
        if (
            len(candidates) > 1
            and valor_pago is not None
            and self.valor_col is not None
        ):
            return self._choose_closest_by_value(candidates, valor_pago)
    ...
    def _try_substring_match(
        self, proc_normalized: str, valor_val: Optional[float] = None
    ) -> Optional[pd.Series]:
        """
        ...
        substring_mask = self._processo_normalized.apply(
            lambda x: (
                (str(x) in proc_normalized) or (proc_normalized in str(x))
                if pd.notna(x)
                else False
            )
        )
    ...
    def _choose_closest_by_value(
        self, candidates: pd.DataFrame, target_value: float
    ) -> pd.Series:
        """
    ...
    def get_process_context(self, mapped_row: pd.Series) -> Dict:
        ...
        return {
            "processo": (
                normalize_process_id(mapped_row.get(self.processo_col))
                if self.processo_col
                else None
            ),
            ...
            "tipo_mercadoria": self._get_value(
                mapped_row, ["tipo de mercadoria", "tipo mercadoria", "tipomercadoria"]
            ),
            "cliente": self._get_value(
                mapped_row, ["cliente", "nome cliente", "nomecliente"]
            ),
            "valor_processo": self._get_value_numeric(
                mapped_row, ["valor realizado", "valor_realizado", "valorrealizado"]
            ),
            "consultor_interno": self._get_value(
                mapped_row, ["consultor interno", "consultorinterno", "consultor"]
            ),
            "representante": self._get_value(
                mapped_row,
                ["representante-pedido", "representante pedido", "representante"],
            ),
            "status_processo": self._get_value(
                mapped_row, ["status processo", "statusprocesso", "status"]
            ),
        }
    ...
    def _get_value_numeric(
        self, row: pd.Series, column_aliases: list
    ) -> Optional[float]:
        """
